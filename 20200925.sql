...
1. ROLLUP(col1,col2...)
    . 기술된 컬럼을 오른쪽에서부터 지원 나가며 서브 그룹을 생성
...
CUBE의 경우 가능 모든 조합으로 서브그룹을 생성하기 때문에
...
1. EMP_TEST 테이블 삭제
2. EMP 테이블을 상요하여 EMP_TEST 테이블 생성(모든 컬럼, 모든 데이터)
3. EMP_TEST 테이블에는 DNAME 컬럼을 추가 (VARCHAR2(14))
4. 상호연관 서브쿼리를 이용하여 EMP_TEST 테이블의 DNAME 컬럼을 DEPT을 이용하여 UPDATE
...
SELECT DECODE(TO_CHAR(TO_DATE('20191229','YYYYMMDD')),'iw'),
       DECODE(TO_CHAR(TO_DATE('20191230','YYYYMMDD')),'iw'),
       DECODE(TO_CHAR(TO_DATE('20191231','YYYYMMDD')),'iw')
FROM dual;
