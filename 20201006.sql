...
RECORD TYPE : 행의 컬럼정보를 개발자가 직접 구상한 (커스텀) 타입
    => 지금까지 타입을 가져다 사용한 것, 지금은 타입을 생성(CLASS를 생성)
방법
TYPE 타입이름 IS RECORD(
    컬럼명1 컬럼타입1,
    컬럼명2 컬럼타입2,
...
배열의 첫 번째 값을 접근 : inArr[0] index 번호 ==> 숫자로 고정 
pl/sql inArr["userName"]
...
v_sal변수에 담고 emp 테이블의 sal 컬럼에 업데이트
단 case 표현식을 사용할 것
