...
분석함수 사용X
SELECT empno, ename, sal, A.deptno, a.MAX_sal
FROM emp,
...
분석함수 사용X 
SELECT empno, ename, sal, a.deptno, a.MIN_sal
FROM emp,
...
실습 ana4]
사원번호, 사원이름, 입사일자, 급여, 급여순위가 자신보다 한 단계 낮은사람의 급여 
...
실습 ana5] 
사원번호, 사원이름, 입사일자, 급여, 급여순위가 자신보다 한 단계 높은사람의 급여 
(단, 급여가 같을 경우 입사일이 빠른 사람이 높은 우선순위)
SELECT empno, ename, hiredate, sal, 
...
이전/이후 n행 값 가져오기
LAG(col [,건널띄 행수 - default 1] [,값이 없을 경우 적용할 기본값])
SELECT empno, ename, hiredate, job, sal,
...
실습 ana7] 급여정보를 부서별로, 급여, 사원번호 오름차순으로 정렬 했을 때
            자신의 급여와 선행하는 급여의 합
...
1. 요청된 SQL과 동일한 SQL이 이미 실행된 적이 있는지 확인하여
    ...
    1-2. 만약 SHARED POOL에 저장된 실행계획이 없다면 (동일한 SQL이 실행됭 적이 없으)
...
후행 트랜젝션이 실핼 트랜젝션에 어떻게 영향을 미치는지를 정의한 단계

LEVEL 0~3
LEVEL 0 : READ UNCOMMITTED 
          선행 트랜젝션이 커밋하지 않은 데이터도 후행 트랜젝션에서 조회된다
          오라클에서는 공식적으로 지원하지 않는 단계
          
LEVEL 1 : READ COMMITTED 
          후행 트랜젝션에서 커밋한 데이터가 선행 트랜젝션에서도 조회된다
          오라클의 기본 고립화 레벨
          대부분의 DBMS가 채택하는 레벨 
          
LEVEL 2 : REPEATABLE READ (반복적 읽기)
          트랜젝션안에서 동일한 SELECT 쿼리를 실행해도 트랜젝션의 어떤 위치에서든지
          후행 트랜젝션의 변경(UPDATE), 삭제 (DELETE)의 영향을 받지 않고 항상 동일한 실행결과를 조회하는 레벨
          
          오라클에서는 공식적으로 지원하지는 않지만 SELECT FOR UPDATE 구문을 통해 효과를 낼 수 있다
          
          후행 트랜젝션의 변경, 삭제에 대해서는 막을 수 있지만
          (테이블에 기존에 존재했던 데이터에 대해)
          신규(INSERT)로 입력하는 데이터에 대해서는 선행 트랜젝션에 영향이 간다
          ==> Phantom Read (귀신 읽기)
              존재하지 않았던 데이터가 갑자기 조회되는 현상
              
LEVEL 3 : SERIALIZABLE READ (직렬화 읽기)
          후행 트랜젝션의 작업이 선행 트랜젝션에 아무런 영향을 주지 않는 단계
          ***DBMS마다 LOCKING메카니즘이 다르기 때문에 ISOLATION LEVEL을 함부로 수정하는 것은 위험하다
          
엑셀(ISOLATION LEVEL)에 시나리오 참고 -- 공유폴더에       