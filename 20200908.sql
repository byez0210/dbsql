...
날짜 : 일자 +시분초
2020년 9월 8일 14시 10분 5초 ==> 20200908 ==> 2020년 9월 8일 00시 0분0초
where cs_rcv_dt TO_DATE
SELECT TO_CHAR(SYSDATE,'YYYYMMDD')
FROM dual;

SELECT TO_CHAR(SYSDATE , 'YYYY-MM-DD')dt_dash, 
       TO_CHAR(SYSDATE , 'YYYY-MM-DD hh24-mi-ss') dt_dash_with_time, 
       --TO_CHAR(TO_DATE('SYSDATE , 'YYYY-MM-DD'),'DD-MM-YYYY') dt_dd_yyyy
...
SELECT  TO_DATE(:yyyymm,'YYYYMM') param,  TO_CHAR(LAST_DAY(TO_DATE( :yyyymm,'YYYYMM')),'DD') dt
FROM dual;
...
묵시적 형변환
    ...
    JAVA시간에 8가지 원시타입(primitive type) 긴 형변환
    1가지 타입이 다른 7가지 타입으로 변환될 수 있다
    8*7 =56가지
    
두가지 가능한 경우
1. empno(숫자)를 문자로 묵시적 형변환
2. '7368'(문자)를 숫자로 묵시적 형변환

알면 매우 좋음, 몰라도 수업 진행하는데 문제없고, 추후 취업해서도 큰 지장없음
다만 고급 개발와 일반 개발자를 구분하는 차이점
...
1. 위에서 아래로
2. 단 자식노드(들여쓰기가된 노드) 있을경우 자식부터 실행하고
    본인 노드실행
...
함수
    ...
    오라클의 NVL 함수와 동일한 역할을 하는 MS-SQL SEVER의 함수이름?
