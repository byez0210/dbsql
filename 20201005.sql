...
1. 인위적으로 여러개의 행을 만들기
    ...
    년월 문자열이 주었졌을 때 해당 월의 일수 구하기
    EX : '202010' ==> 31
        날짜가 있으면 원하는 항목(년, 월, 일, 시, 분, 초)만 추출할 수 있다.
        TO_CHAR(날짜,'원하는 항목')
        TO_CHAR(해당 날짜의 마지막 날짜 구하는 함수(TO_DATE('202010', '포맷')), '원하는 항목')
        ...
        DATE + 정수 = DAT에서 정수를 일자로 취급해서 더한 날짜
        ...
        ==> LEVEL은 1부터 시작하므로 2020년 10월 1일 값을 유지하기 위해서는
        날짜 + LEVEL -1
       ...
       해당 날짜의 요일에 따라서 새롭게 만든 7개 컬럼(일~월)에 날짜를 위치시킨다.
       
       SELECT /*day, d, iw, DECODE(d, 1, iw+1 ,iw) iw,*/
...
PL/SQL block 구조 : JAVA try catch와 유사 - 중첩 가능 
DECLARE
    선언부(생략가능) 
          - PL/SQL블럭에서 사용할 변수, TYPE(CLASS), CURSOR(SQL-정보)등을 선언하는 절
            JAVA랑은 다르게 변수언선을 블록 어디서나 할 수 없음
BEGIN 
    실행부(생략불가)
...
pl/sql 식별자 규칙 : 오라클 객체(table, index...) 생성기와 동일
...
PL/SQL : DBMS_OUTPUT_.PUT_LINE(...);

ORACLE 결과출력을 위해 출력 기능을 활성화 해야함
매번 실행할 필요는 없고, 오라클 접수 후 한번만 실행하면 됨
(내일 수업시 다시 실행)
...
printdept 프로시저는 begin절에 10번부서의 정보를 조회하도록
hard coding 되어있음
프로시저가 인자를 받도록 수정
