Exception (예외) : pl/sql 블럭이 실행되는 동안 발생하는 에러
                    예외가 발생했을 때 pl/sql의 EXCEPTION 절을 기술하여
                    다른 로직을 실행하여 예외문제를 해결할 수 있다.
예외 구분
1. 사전 정의 예외(Oracle Predifined Exception)
    오라클에서 미리 정의한 예외로 ORA-XXXX로 정의
    에러코드만 있고 에러 이름이 없는 경우가 대다수
    
2. 사용자 정의 예외(User Defined Exception)

예외 발생 시 해당 sql문은 중단
        . EXCEPTION 절이 있으면 : 예외 처리부에 기술된 문장을 실행
        . EXCEPTION 절이 없으면 : PL/SQL블럭 종료
...
사용자 예외 정의, 사용 
1. PL/SQL 블럭안에서 또 다른 PL/SQL 블럭을 정의하는 것이 가능

NO_EMP : 사번을 통해서 사원을 검색하는 해당 사번을 갖는 사원이 없을 때
    SELECT *
    FROM emp
    WHERE empno = -1;
    ==> NO_DATA_FOUND ==> N0_EMP
...
FROM emp;
데이터 분포도(20개 이하)가 나쁘면 함수를 쓰는 것이 좋다 

분포도 : 흩어진 정도
데이터 분포도 : 특정 데이터가 발생하는 빈도
예 : emp테이블의 empno : 값이 14개 존재 - 데이터 분포도가 좋다
    emp테이블의 deptno : 값이 3개 존재 - 데이터 분포도가 나쁘다

함수의 경우 오라클에서 기본적으로 캐쉬 기능을 사용
기본 캐쉬 사이즈가 20개 
getDeptName(10) => ACCOUNTING 
오라클은 10번인자로 getDeptName 실행했을 때 ACCOUNTING이라는 결과 값을 기억(캐싱)
...
패키지 생성 방법 : java의 interface, class 사용방법과 유사
...
Trigger(방아쇠) : 설정한 이벤트에 따라 실행할 로직을 담은 객체
    웹 : 로그인 버튼 클릭
            ==> 사용자 아이디 input 값과, 비밀번호 input 값을
                파라미터로 서버로 전송
    db : 테이블의 데이터가 추가되거나 변경되었을 때 PL/SQL 블럭을 실행
         user 테이블에 사용자 비밀번호가 변경되었을 때
         user_history 테이블에 기존에 사용하던 비밀번호 이력으로 생성
...
BEGIN
    --테이블 업데이트가 일어났을 때, 
    --기존(:OLD.PASS) 비밀번호와 업데이트하려고 하는 비밀번호(:NEW.pass)가 다를 때
...
trg_users)pass 트리거에 의해 users_history 테이블에 비밀번호 이력이 생긴 것을 볼 수 있다.
SELECT *
FROM users_history;
...
트리거 
    ...
    장점(시스템 신규로 만드는 사람 SI) 
        : users테이블에 비밀번호를 바꾸는 로직(java 코드)은 작성을 해야 한다
          users_history에 이력을 남기는 로직(java 코드)은 작성하지 않아도 됨
    단점(시스템을 유지보수하는 사람 SM)
