...
1. 자동생성
    ...
    DBMS입장에서는 신규데이터가 입력되거나 기본 데이터가 수정된 때
    UNIQUE 제약에 문제가 없는지 확인 ==> 무결성을 지키기 위해
    
    빠른 속도로 중복 데이터 검증을 위해 오라클에서는 unique, PRIMARY KEY 생성 시
    해당 컬럼으로 인덱스 생성을 강제한다
    
    PRIMARY KEY 제약조건 생성 후 해당 인덱스 삭제 시도 시 삭제 불가
    
    FOREIGN KEY : 입력하려는 데이터가 참조하는 테이블의 컬럼에 존재하는 데이터만 입력되도록 제한 
    emp 테이블에 brown 사원을 50번 부서에 입력을 하게되면 50 부서가 dept테이블의 deptno컬럼에 존재여부를 확인하여 
...
5) deptno(=),mgr 컬럼이 있을 경우 테이블 엑세스 불필요
   empno(=)(1)
...
인덱스 사용에 있어서 중요한 점
인덱스 구성컬럼이 모둔 NULL이면 인덱스에 저장을 하지 않는다
즉 테이블 접근에 해봐야 해당 행에 대한 정보를 알 수 있다
가급적이면 컬럼에 값이 NULL이 들어오지 않는 경우는 NOT NULL 제약을 적극적으로 활용
...
dictionary는 크게 4가지 로 구분 가능
USER_ : 해당 사용자가 소융한 객체만 조회
ALL_ : 해당 사용자가 소유한 객체 + 다른 사용자로부터 권한을 부여받은 객체 조회
DBA_ : 시스템 관리자만 볼 수 있으면, 모든 사용자의 객체 조회
...
기본에 배운 쿼린 하나의 테이블에 여러건의 데이터를 입력하는 쿼리
INSERT INTO emp (empno,ename)
SELECT empno, ename
FROM emp;

multiple insert 구분
 1 .unconditional insert : 여러 테이블의 insert
 2. conditional insert : 조건을 만족하는 모든 테이블에 insert
 3. conditional first insert : 조건을 만족하는 첫번 째 테이블에 insert
...
uncondition insert 실행 시 테이블마다 데이터를 입력할 컬럼을 조작하는 것이 가능
위에서 : INSERT INTO emp_test VALUES (..) 테이블의 모든 컬럼에 대해 입력
        INSERT INTO emp_test (empno) VALUES(9999)특정 컬럼에 지정하여 입력 가능
...
CASE
    WHEN job = 'MANAHER' THEN sal* 1.05
