...
2. 자주 사용되는 쿼리를 view 만들어서 재사용
    ex : emp 테이블은 dept 테이블이랑 조인되서 사용되는 경우가 많음
        view를 만들지 않을경우 매번 조인 쿼리를 작성해야 하나, view로만 들면 재사용이 가능
...
사용방법 : 함수를 생각하자
함수테스트 : dual
시퀀스 객체명.nextval : 시퀀스 객체에서 마지막으로 사용한 다음 값 반환
시퀀스 객체명.currval : nextval 함수를 실행하고 나서 사용할 수 있다
                      nextval 함수를 통해 얻어진 값을 반환
SELECT seq_emp.nextval --1=>2=>..
FROM dual;

SELECT seq_emp.currval --seq_emp.nextval의 값을 반환
FROM dual;
...
시퀀스를 롤백을 하더라도 읽은 값이 복원되지 않는다
CREATE SEQUENCE SEQ_EMP;


INDEX : TABLE 의 일부 컬럼을 기분으로 미리 정렬해둔 객체
ROWID : 테이블에 저장된 행의 위를 나타내는 값

SELECT ROWID,empno,ename
FROM emp;

만약 ROWID를 알수만 있다면 해당 테이블의 모든 데이터를 뒤지지 않아도 
해당 행에 바로 접근 할 수 있다
EXPLAIN PLAN FOR
SELECT  *
FROM emp
WHERE ROWID = 'AAAE5hAAFAAAACNAAA';
...
block의 크기는 데이터베이스 생성 시 결정, 기본값 8K byte
DESC emp;
emp테이블 해당 행은 최대 54byte
...
==>테이블에는 순서가 없기 때문에 emp 테이블의 14건의 데이터를 모두 뒤져보고 
    empno 값이 7782인 한건에 대해서만 사용자에게 반환한다
...
시나리오4
emp 테이블의 JOB컬럼으로 non_unique 인덱스를 생성한 경우
...
---------------------------------------------------
   ...
   1 - filter("ENAME" LIKE 'C%')  -- filter 테이블로 가서 맞는지 확인 
   2 - access("JOB"='MANAGER')
...
시나리오8
emp 테이블이 empno 컬럼에 UNIQUE 인덱스 생성
dept 테이블이 deptno 컬럼에 UNIQUE 인덱스 생성
